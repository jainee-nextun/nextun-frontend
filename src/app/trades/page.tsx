          ...
          <div className="bg-white dark:bg-[#1E293B] rounded-lg shadow-sm py-4 mt-4">
           <div className="flex flex-wrap items-center justify-start gap-3 sm:gap-4 mb-4 px-4 w-full">
            <div className="relative w-auto sm:w-auto dark:bg-[#1E293B]" style={{ minWidth: 150 }}>
              ...
              <span className="dark:bg-[#1E293B] pointer-events-none absolute right-4 top-1/2 transform -translate-y-1/2">
              ...
              <table className="min-w-[750px] w-full text-sm table-auto">
                <thead className="bg-[#F2F7FF] dark:bg-[#293850] dark:text-white">
                  <tr>
                    ...
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">Type</th>
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">Quantity</th>
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">Entry Price</th>
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">Current Price</th>
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">P&amp;L</th>
                    <th className="px-2 sm:px-4 py-2 text-left font-medium text-[#0E3D66] dark:text-white text-xs sm:text-sm">Status</th>
                  </tr>
                </thead>
                <tbody className='text-[#0E3D66] dark:text-gray-100'>
                  {paginated.map((trade, i) => (
                    <tr key={i} className="border-t last:border-b">
                      <td className="px-2 sm:px-4 py-2 whitespace-nowrap text-[#0E3D66] dark:text-white">{trade.symbol}</td>
                ...
                {getPages().map((n, idx) =>
                  n === '...'
                    ...
                    : <button
                        ...
                        className={`px-3 py-1 ${
                          page === n 
                           ? 'bg-blue-600 text-white dark:bg-[#0f172a]' 
                          : 'bg-white dark:bg-[#293850] text-black dark:text-white'
                        } border-r font-normal border-gray-300 last:border-r-0`}

                      >{n}</button>
                )}
                <button onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page === totalPages} className="px-3 py-1 disabled:opacity-50 bg-white dark:bg-[#293850] dark:text-white">
                  {'>'}
                </button>
              </div>
            </div>
